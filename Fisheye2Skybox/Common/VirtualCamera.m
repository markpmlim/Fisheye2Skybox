...
@implementation VirtualCamera {
    ...
    quaternion_float _previousQuat;
    quaternion_float _currentQuat;
}
...
- (instancetype) initWithScreenSize:(CGSize)size {
    ...
    if (self != nil) {
        ...
        _orientation  = quaternion_identity();
        _previousQuat = quaternion_identity();
        _currentQuat  = quaternion_identity();
...
- (quaternion_float)rotationBetweenVector:(vector_float3)from
                                andVector:(vector_float3)to
{
    vector_float3 u = vector_normalize(from);
    vector_float3 v = vector_normalize(to);

    // Angle between the 2 vectors
    float cosTheta = vector_dot(u, v);
    ...
    if (cosTheta < -1 + 0.001f) {
        ...
        rotationAxis = vector_cross(vector_make(0.0f, 0.0f, 1.0f), u);
        float length2 = vector_dot(rotationAxis, rotationAxis);
        if ( length2 < 0.01f ) {
            // Bad luck, they were parallel, try again!
            rotationAxis = vector_cross(vector_make(1.0f, 0.0f, 0.0f), u);
        }

        rotationAxis = vector_normalize(rotationAxis);
        //return simd_quaternion(radians_from_degrees(180.0f), rotationAxis);
        //return quaternion(radians_from_degrees(180.0f), rotationAxis);
        return quaternion_from_axis_angle(rotationAxis, radians_from_degrees(180.0f));
    }

    // Compute rotation axis.
    rotationAxis = vector_cross(u, v);

    float angle = acosf(cosTheta);
    // Normalising the axis should produce a unit-norm quaternion.
    rotationAxis = vector_normalize(rotationAxis);
    quaternion_float q = quaternion(-angle, rotationAxis);
...
- (void)dragToPoint:(CGPoint)point
{
    ...
    _endPoint = [self projectMouseX:mouseX
                               andY:mouseY];
    quaternion_float delta = [self rotationBetweenVector:_startPoint
                                               andVector:_endPoint];
    //_currentQuat = simd_mul(delta, _previousQuat);
    _currentQuat = quaternion_multiply(delta, _previousQuat);
}
